...
class FeatureViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val state = combine(settings.dailyTarget, dao.observeWeights(), dao.observeEarnedAchievements()) { target, weights, earned ->
        FeatureUiState(
            target = target,
            weights = weights,
    ...
    fun setTarget(value: Int): Result<Unit> {
        ...
        viewModelScope.launch(Dispatchers.IO) { settings.setDailyTarget(value.toDouble()) }
