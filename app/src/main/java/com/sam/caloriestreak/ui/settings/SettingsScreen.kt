...
@Composable
fun SettingsScreen(target: Double, onSaveTarget: (Int) -> Result<Unit>) {
    ...
    Column(Modifier.padding(16.dp)) {
        ...
        message?.let { Text(it) }
        Text("Freeze rule: 85% qualifying score; current app streak policy remains unchanged.", Modifier.padding(top = 20.dp))
        Text("Theme and notification controls will use the existing app settings when those options are exposed.")
    }
}
