...
private fun WeightHistoryList(weights: List<WeightEntryEntity>) {
    ...
    LazyColumn(contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        items(sorted, key = { it.id }) { entry ->
            Card { ListItem(headlineContent = { Text("%.1f kg".format(entry.kilograms)) }, supportingContent = { Text(DateFormat.getDateTimeInstance().format(Date(entry.timestamp)) + (entry.note?.let { "\n$it" } ?: "")) }) }
        }
    }
}
