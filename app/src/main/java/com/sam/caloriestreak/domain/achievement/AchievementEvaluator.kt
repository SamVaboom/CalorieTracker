...
object AchievementEvaluator {
    ...
    fun eligibleCalorieAndScoreAchievements(daily: List<DailyLogEntity>): Set<String> {
        ...
        val perfect = consecutiveRuns(finalized.filter { abs(it.score - 100.0) < 0.000001 }.map { it.dateEpochDay })
        if (perfect >= 2) result += "double_bullseye"
        if (perfect >= 3) result += "hat_trick"
    ...
    fun eligibleMealAndRecipeAchievements(meals: List<MealLogEntity>, recipeCount: Int, ingredientCount: Int): Set<String> {
        ...
        if (byRecipe.values.any { consecutiveRuns(it.map { log -> log.dateEpochDay }) >= 3 }) result += "meal_prepper"
    ...
    fun eligibleFreezeAchievements(daily: List<DailyLogEntity>, currentFreezes: Int, maxFreezes: Int = 3): Set<String> {
        ...
        if (qualifying >= 7) result += setOf("winter_is_coming", "seven_good_days")
